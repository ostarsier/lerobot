...
可视化 LeRobotDataset 类型数据集的所有帧的任何 episode 的数据。

注意：episode 的最后一帧并不总是对应于最终状态。
这是因为我们的数据集由从一个状态到另一个状态的转换组成，直到与最终动作相关联的倒数第三个状态，以到达最终状态。
然而，可能不存在从最终状态到另一个状态的转换。

注意：此脚本旨在可视化用于训练神经网络的数据。
眼见为实。当可视化图像模态时，通常会观察到有损压缩伪影，因为这些图像是从压缩的 mp4 视频解码而来以节省磁盘空间。应用的压缩因子经过调整，不会影响成功率。
